...
const Feedback = ({ queryId, concepts }) => {
  ...
  const handleStarRating = (type, rating) => {
    setFeedback(prev => ({
      ...prev,
      [type]: rating
    }));
  ...
  const handleConceptFeedback = (conceptName, isHelpful) => {
    setFeedback(prev => ({
      ...prev,
              ...
              <div className="concept-checkboxes">
                {concepts.map((concept, index) => (
                  <label key={index} className="checkbox-item">
                    <input
                      type="checkbox"
                      onChange={(e) => handleConceptFeedback(concept.name, e.target.checked)}
                    />
                    {concept.name}
                  </label>
                ))}
